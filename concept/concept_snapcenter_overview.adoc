...
+
Para obter mais informações sobre cmdlets do Windows, https://docs.netapp.com/us-en/snapcenter-cmdlets-50/index.htmllllll["Guia de referência de cmdlet do software SnapCenter"^]consulte .

+
Para obter mais informações sobre comandos UNIX, https://library.netapp.com/ecm/ecm_download_file/ECMLP2886896["Guia de Referência de comandos do software SnapCenter"^]consulte .
...
+
O SnapMirror Business Continuity (SM-BC) permite que os serviços de negócios continuem operando mesmo em caso de falha completa do local, com suporte ao failover de aplicações de forma transparente, usando uma cópia secundária. Nem a intervenção manual nem o script adicional são necessários para acionar um failover com o SM-BC.

+
Os plug-ins suportados para esse recurso são o plug-in SnapCenter para SQL Server, o plug-in SnapCenter para Windows e o plug-in SnapCenter para banco de dados Oracle.

+
Para obter mais informações sobre SM-BC, consulte https://docs.netapp.com/us-en/ontap/smbc/index.html["Continuidade dos negócios do SnapMirror (SM-BC)"]

+
Para o SM-BC, certifique-se de que você atendeu aos vários requisitos de configuração de hardware, software e sistema. Para obter mais informações, consulte https://docs.netapp.com/us-en/ontap/smbc/smbc_plan_prerequisites.html["Pré-requisitos"]
